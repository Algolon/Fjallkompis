...
6. ~~**Worn clothing follow-ups**~~ — resolved: real-device packing showed
   the row-level Worn marking misstated weights for wear-one-carry-rest
   rows (shirts, socks, underwear), so the unit-level `wornQuantity` model
